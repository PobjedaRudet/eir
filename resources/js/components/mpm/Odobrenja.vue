          ...
          <div class="flex gap-2 flex-wrap">
            <template v-if="rejectingOrderId === order.id">
              <button @click="submitRejectOrder(order.id)" :disabled="actioning"
                ...
                {{ actioning ? 'Šalje se...' : 'Potvrdi odbijanje' }}
              </button>
...
async function loadOrders() {
  ...
  try {
    const res  = await fetch(`${BASE}/api/mpm/orders/pending`, { headers: { Accept: 'application/json' } })
...
async function approveOrder(id) {
  ...
  try {
    const res = await fetch(`${BASE}/api/mpm/orders/${id}/approve`, { method: 'POST', headers: hdrs() })
...
async function submitRejectOrder(id) {
  ...
  actioning.value = true; actionError.value = ''
  try {
    const res = await fetch(`${BASE}/api/mpm/orders/${id}/reject`, {
