      ...
      <div v-for="day in days" :key="day.date">
        ...
        <div class="flex items-center gap-3 mb-4">
          <div class="flex items-center gap-2 rounded-lg bg-blue-600 px-3 py-1.5 text-white">
            <svg class="size-4 shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M5.75 2a.75.75 0 0 1 .75.75V4h7V2.75a.75.75 0 0 1 1.5 0V4h.25A2.75 2.75 0 0 1 18 6.75v8.5A2.75 2.75 0 0 1 15.25 18H4.75A2.75 2.75 0 0 1 2 15.25v-8.5A2.75 2.75 0 0 1 4.75 4H5V2.75A.75.75 0 0 1 5.75 2Zm-1 5.5c-.69 0-1.25.56-1.25 1.25v6.5c0 .69.56 1.25 1.25 1.25h10.5c.69 0 1.25-.56 1.25-1.25v-6.5c0-.69-.56-1.25-1.25-1.25H4.75Z" clip-rule="evenodd" /></svg>
            <span class="text-sm font-semibold whitespace-nowrap">{{ formatDate(day.date) }}</span>
          </div>
          <span class="px-2 py-0.5 text-xs rounded-full bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300">
            {{ day.entries.length }} {{ day.entries.length === 1 ? 'unos' : 'unosa' }}
          </span>
          <span class="px-2 py-0.5 text-xs rounded-full bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300">
            🔧 {{ day.entries.reduce((s, e) => s + e.operations.length, 0) }} op.
          </span>
          ...
          >
            ...
            <div class="flex flex-wrap items-center gap-x-3 gap-y-2 px-5 py-3 bg-neutral-50 dark:bg-neutral-800 border-b border-neutral-200 dark:border-neutral-700">
              <div class="flex items-center gap-1.5">
                <svg class="size-4 text-neutral-400 shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M18 10a8 8 0 1 1-16 0 8 8 0 0 1 16 0Zm-5.5-2.5a2.5 2.5 0 1 1-5 0 2.5 2.5 0 0 1 5 0ZM10 12a5.99 5.99 0 0 0-4.793 2.39A6.483 6.483 0 0 0 10 16.5a6.483 6.483 0 0 0 4.793-2.11A5.99 5.99 0 0 0 10 12Z" clip-rule="evenodd" /></svg>
                <span class="text-sm font-semibold text-neutral-800 dark:text-neutral-200">{{ entry.worker }}</span>
              </div>
              <span class="text-neutral-300 dark:text-neutral-600 hidden sm:inline">·</span>
              <div class="flex items-center gap-1.5">
                <svg class="size-4 text-neutral-400 shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path d="M3.75 3A1.75 1.75 0 0 0 2 4.75v3.26c.313-.141.65-.26 1-.356V4.75c0-.138.112-.25.25-.25h13c.138 0 .25.112.25.25v3.654c.35.096.687.215 1 .356V4.75A1.75 1.75 0 0 0 16.25 3H3.75Z" /><path d="M4.75 7.25c-.69 0-1.25.56-1.25 1.25v5.75c0 .138.112.25.25.25h2.5a.75.75 0 0 0 .75-.75v-2.25a2.25 2.25 0 0 1 4.5 0v2.25c0 .414.336.75.75.75h2.5a.25.25 0 0 0 .25-.25V8.5c0-.69-.56-1.25-1.25-1.25H4.75Z" /></svg>
                <span class="text-sm text-neutral-600 dark:text-neutral-400">{{ entry.project }}</span>
              </div>
              <template v-if="entry.street">
                <span class="text-neutral-300 dark:text-neutral-600 hidden sm:inline">·</span>
                <span class="text-sm text-neutral-500 dark:text-neutral-400">{{ entry.street }}</span>
              </template>
              <template v-if="entry.enclosure">
                <span class="text-neutral-300 dark:text-neutral-600 hidden sm:inline">·</span>
                <span class="text-sm text-neutral-500 dark:text-neutral-400">Kućište: {{ entry.enclosure }}</span>
              </template>
              <div class="ml-auto flex flex-wrap items-center gap-1">
                <span class="px-2 py-0.5 text-xs rounded-full border border-neutral-200 dark:border-neutral-700 text-zinc-600 dark:text-zinc-300">{{ entry.cable_type }}</span>
                <span
            ...
            <div class="divide-y divide-neutral-100 dark:divide-neutral-800">
              <div v-for="(op, opIdx) in entry.operations" :key="op.id" class="px-5 py-4">
                <div class="flex flex-wrap items-center gap-2 mb-3">
                  ...
                  <template v-if="op.kind === 'iskop'">
                    ...
                    <span v-if="op.meterage" class="px-2 py-0.5 text-xs rounded-full bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300">↔ {{ parseFloat(op.meterage).toFixed(2) }} m</span>
                  </template>
              ...
              <div v-if="!entry.operations.length" class="px-5 py-4 text-sm text-neutral-400">Nema operacija.</div>
            </div>
          </div>
        </div>
      </div>
